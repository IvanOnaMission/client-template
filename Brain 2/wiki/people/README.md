...
2. The AI loads **this folder** + the shared brain tiers their role is allowed to see.
...
Treat `people/` as **admin-tier**. One person's folder is **never loaded into another person's session.** The AI writes ONLY to the folder of whoever is currently talking. This is enforced in the agent's code, not in GitHub (GitHub can't do folder-level access — see [[Brain 2/wiki/concepts/aios-architecture]]).
...
- [ ] Role/tier assigned — which shared tiers this person can read (staff / admin / owner)
- [ ] Confirmed the agent writes **only** to this folder for this person's session
- [ ] **No credentials anywhere** in the folder (see `../ACCESS-MODEL.md`)
