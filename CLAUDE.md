...
synthesised pages in `Brain 2/wiki/`. **Read `Brain 2/wiki/index.md` to look something up.**
Full schema: `Brain 2/CLAUDE.md`.

## Rules for any agent
1. Obey `Brain 1/boundaries.md` before drafting, sending, quoting, or invoicing.
2. For a person-specific task, read `Brain 2/wiki/people/{name}/`.
3. One topic per file. No giant docs. Never commit credentials (status only,
   `Brain 2/wiki/access-tracker.md`).
4. Fill **just-in-time** — only what the current module needs. If the brain is wrong or
   missing something, tell the human — don't guess.
5. `Brain 2/raw/` sources are **immutable** — never edit them.
6. **Onboard new people automatically.** At the start of a session with someone whose `Brain 2/wiki/people/{name}/` folder is missing or still has `[Person Name]`/placeholder content, run the **`onboard-person`** skill before doing anything else — interview them and fill their files. Don't make a new person work with a blank brain.
